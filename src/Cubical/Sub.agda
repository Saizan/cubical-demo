...
module _ {ℓ} {A : Set ℓ} where
  ouc-φ : {t : Partial i1 A} → (s : A [ i1 ↦ t ]) → (ouc s) ≡ t itIsOne
  ouc-φ s i = ouc s
...
Comp : {ℓ : I → Level} → (A : (i : I) → Set (ℓ i)) → (φ : I) →
       (u : (i : I) → Partial φ (A i)) → A i0 [ φ ↦ u i0 ]
                                       → A i1
