...
-- Functor
record Functor {o1 h1 o2 h2} (C : Category {o1} {h1})(D : Category {o2} {h2}) : Set (ℓ-suc (ℓ-max (ℓ-max o1 h1) (ℓ-max o2 h2))) where
  no-eta-equality
  constructor con
  private
    module C = Category C
...
_∘ᶠ_ : ∀ {oa ha ob hb oc hc} {A : Category {oa} {ha}} {B : Category {ob} {hb}} {C : Category {oc} {hc}}
...
(G ∘ᶠ F) .presComp f g = trans (cong (G .hom) (F .presComp f g)) (G .presComp (F .hom f) (F .hom g))



-- Functors into a 1-cat are equal iff their actions on objects and arrows are equal.

Func≡ : ∀ {o1 h1 o2 h2} {C : Category {o1} {h1}} {D : Category {o2} {h2}} → 1-cat D → {F G : Functor C D}
        → (eq : F .obj ≡ G .obj)
        → PathP (\ i → ∀ (X Y : C .Obj) → Hom C X Y → Hom D (eq i X) (eq i Y)) (\ _ _ → F .hom) (\ _ _ → G .hom)
        → F ≡ G
Func≡ {C = C} {D} 1-D {F} {G} eq p = r where
  mutual
   H : (X : _) → PathP (λ z → PathP (λ _ → Hom D (eq z X) (eq z X)) (p z _ _ (C .id)) (D .id))
                       (F .presId {X}) (G .presId {X})
   H X = toPathP (1-D _ _ _ _ _ _)
   H2 : ∀ {A B C1} → (f : Hom C A B) (g : Hom C B C1) →
                  PathP (\ i → p i _ _ (C ._∘_ g f) ≡ (D ∘ p i _ _ g) (p i _ _ f)) (F .presComp f g) (G .presComp f g)
...
record NaturalTransformation
  ...
  field
    ...
    natural : ∀ {X Y} (f : Hom C X Y) → apply {Y} D.∘ F .hom f ≡ G .hom f D.∘ apply {X}
...
NT= : ∀ {o1 h1 o2 h2} {C : Category {o1} {h1}} {D : Category {o2} {h2}} (let module D = Category D)
       {F G : Functor C D} {f g : NaturalTransformation F G} →
       (eq : Path {A = ∀ {X} → Hom D (F .obj X) (G .obj X)} (f .apply) (g .apply)) → 1-cat D → f ≡ g
NT= eq p i .apply = eq i
NT= {D = D} {F} {G} {a} {b} eq p i .natural {X} {Y} f
  = toPathP
      {A = λ i₁ →
         PathP (λ _ → D .Hom (F .obj X) (G .obj Y))
               (D ._∘_ (eq i₁) (F .hom f)) (D ._∘_ (G .hom f) (eq i₁))}
...
module Model (C : Category {ℓ-zero} {ℓ-zero}) where
  ...
  substElem σ f = trans (sym (let H = _ in \ i → σ .natural (f .fst) i H)) (cong (σ .apply) (f .snd))
